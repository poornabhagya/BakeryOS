...
import { Search, X, Plus, FileText, Printer } from 'lucide-react';
...
const products: Product[] = [
  {
    ...
    price: 80,
    stock: 10,
    image: 'https://images.unsplash.com/photo-1756137943021-6f2ffaa646da?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxmaXNoJTIwYnVuJTIwcGFzdHJ5fGVufDF8fHx8MTc2ODgzODYxMHww&ixlib=rb-4.1.0&q=80&w=1080',
    category: 'Buns',
    itemId: '#B001',
  },
  {
    ...
    price: 60,
    stock: 15,
    image: 'https://images.unsplash.com/photo-1741004420618-209c60a86d58?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHx0ZWElMjBidW4lMjBicmVhZHxlbnwxfHx8fDE3Njg4Mzg2MTF8MA&ixlib=rb-4.1.0&q=80&w=1080',
    category: 'Buns',
    itemId: '#B002',
  },
  {
    ...
    price: 450,
    stock: 8,
    image: 'https://images.unsplash.com/photo-1700448293876-07dca826c161?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxjaG9jb2xhdGUlMjBjYWtlJTIwc2xpY2V8ZW58MXx8fHwxNzY4ODA2NzgwfDA&ixlib=rb-4.1.0&q=80&w=1080',
    category: 'Cakes',
    itemId: '#C001',
  },
  {
    ...
    price: 120,
    stock: 20,
    image: 'https://images.unsplash.com/photo-1712723246766-3eaea22e52ff?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxjcm9pc3NhbnQlMjBwYXN0cnl8ZW58MXx8fHwxNzY4ODEyNzg4fDA&ixlib=rb-4.1.0&q=80&w=1080',
    category: 'Buns',
    itemId: '#P001',
  },
  {
    ...
    price: 150,
    stock: 30,
    image: 'https://images.unsplash.com/photo-1564676677001-92e8f1a0df30?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxjb2ZmZWUlMjBjdXAlMjBkcmlua3xlbnwxfHx8fDE3Njg4MDk4ODR8MA&ixlib=rb-4.1.0&q=80&w=1080',
    category: 'Drinks',
    itemId: '#D001',
  },
  {
    ...
    price: 90,
    stock: 12,
    image: 'https://images.unsplash.com/photo-1663904460424-91895028aa9e?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxicmVhZCUyMGxvYWYlMjBmcmVzaHxlbnwxfHx8fDE3Njg4Mzg2MTJ8MA&ixlib=rb-4.1.0&q=80&w=1080',
    category: 'Bread',
    itemId: '#BR01',
  },
];
  ...
  const addToCart = (product: Product) => {
    ...
    if (existingItem) {
      if (existingItem.quantity >= product.stock) {
        setStockError(`Insufficient Stock for ${product.name}! (Max: ${product.stock})`);
    ...
    setCart(cart.map(item => {
      if (item.id === id) {
        ...
        if (newQuantity > item.stock) {
          setStockError(`Insufficient Stock for ${item.name}! (Max: ${item.stock})`);
          setTimeout(() => setStockError(null), 5000);
          return item;
        }
  ...
  const handleCheckout = () => {
    ...
    const payload = {
      ...
      items: cart.map(item => ({
        ...
        unit_price: item.price,   // Frozen price — captured when product was added to cart
      })),
    };
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {filteredProducts.map(product => (
                      ...
                      >
                        <td className="px-6 py-4 text-sm text-gray-600">{product.itemId}</td>
                        <td className="px-6 py-4 text-gray-900">{product.name}</td>
                        <td className="px-6 py-4 text-gray-900 tabular-nums">{product.stock}</td>
                        <td className="px-6 py-4 text-orange-700 tabular-nums">Rs. {product.price}</td>
                        <td className="px-6 py-4">
                          <button
                            onClick={() => addToCart(product)}
                            disabled={product.stock === 0 || !isCounterOpen}
                            className={`px-4 py-2 rounded-lg flex items-center gap-1 transition-colors ${
                              product.stock === 0 || !isCounterOpen
